...
const SubscriberInfoPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const join = async () => {
      try {
        await connection.invoke('JoinSubscriberGroup');
      } catch {
        // ignore
      }
    };
    ...
    const onUpdated = (payload: unknown) => {
      ...
      const updated = payload as SubscriberMaintenanceRequestDto;
      if (!updated.id) return;
      queryClient.setQueryData<SubscriberMaintenanceRequestDto[]>(
        ['subscriber-app', 'maintenance'],
        (old) => {
          if (!old) return old;
    ...
    connection.on('subscriberMaintenanceRequestUpdated', onUpdated);
    ...
    return () => {
      stopped = true;
      connection.off('subscriberMaintenanceRequestUpdated', onUpdated);
      connection.stop().catch(() => undefined);
      void stopped;
    };
  }, [isLoggedIn, queryClient, session?.subscriberId]);
